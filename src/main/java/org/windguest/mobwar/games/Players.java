...
public class Players {
    ...
    private static final Map<UUID, Map<TaskTag, List<BukkitTask>>> playerTasks = new HashMap<>();
    ...
    public static void addTaskToPlayer(Player player, BukkitTask task, TaskTag tag) {
        UUID uuid = player.getUniqueId();
        playerTasks.computeIfAbsent(uuid, k -> new EnumMap<>(TaskTag.class))
                .computeIfAbsent(tag, k -> new ArrayList<>()).add(task);
    }

    public static void cancelPlayerTasks(Player player, TaskTag tag) {
        UUID uuid = player.getUniqueId();
        Map<TaskTag, List<BukkitTask>> map = playerTasks.get(uuid);
        if (map == null) return;
        List<BukkitTask> list = map.remove(tag);
        if (list != null) {
    ...
    public static void cancelPlayerAllTasks(Player player) {
        UUID uuid = player.getUniqueId();
        Map<TaskTag, List<BukkitTask>> map = playerTasks.remove(uuid);
        if (map != null) {
            map.values().forEach(tasks -> tasks.forEach(BukkitTask::cancel));
        }
    }
    ...
    public static void setPlayerFly(Player player, float ascentSpeed) {
        ...
        addTaskToPlayer(player, task, TaskTag.SKILL);
    }
