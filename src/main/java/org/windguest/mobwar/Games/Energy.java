...
public class Energy {
    ...
    public static void createBar(Player player, Material material, String name, BarColor color, double totalEnergy, double initialEnergy) {
    ...
    private static void addEnergy(Player player, String name, double energy) {
        ...
        if (newEnergy >= totalEnergy) {
            ...
            player.spawnParticle(Particle.TOTEM_OF_UNDYING, player.getLocation().add(0, 1, 0), 100, 0.5, 1, 0.5, 0.0, null, true);
    ...
    public static void addEnergyToAllBars(Player player, double energy) {
        ...
        if (bossBars != null) {
            for (Map.Entry<String, Data> entry : bossBars.entrySet()) {
                ...
                if (isDecaying(bossBarData)) continue;
    ...
    private static boolean isDecaying(Data bossBarData) {
        return bossBarData.getDecayTask() != null;
    }
    ...
    public static void startDecay(Player player, String name, int t) {
        ...
        if (totalTicks <= 0) totalTicks = 1; // 防止除以零
