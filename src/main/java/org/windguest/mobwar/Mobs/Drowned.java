...
public class Drowned implements Listener {
    ...
    public void onTridentHit(ProjectileHitEvent event) {
        if (event.getEntity() instanceof Trident trident) {
            if (trident.getShooter() instanceof Player shooter) {
                ...
                for (Entity entity : world.getNearbyEntities(strikeLocation, 2.0, 2.0, 2.0)) {
                    if (entity.equals(shooter)) return;
                    if (entity instanceof Player p) {
                        if (Players.isTeam(shooter, p)) return;
                    }
                    if (entity instanceof LivingEntity livingEntity) {
                        livingEntity.damage(8, shooter);
                    }
                }
            }
        }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (item != null) {
            ...
            if (itemMeta != null) {
                if (itemType == Material.NAUTILUS_SHELL && itemMeta.getDisplayName().equals("§9忠诚") && Players.getJobFromPlayer(player).equals("溺尸")) {
                    ...
                    Energy.startDecay(player, "§9忠诚", 15);
                } else if (itemType == Material.LIGHTNING_ROD && itemMeta.getDisplayName().equals("§c天雷") && Players.getJobFromPlayer(player).equals("溺尸")) {
                    event.setCancelled(true);
                    ...
                    Energy.startDecay(player, "§c天雷", 20);
                }
            }
        }
    }
